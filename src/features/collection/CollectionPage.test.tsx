...
describe('CollectionPage global search capture', () => {
  ...
  it('uses compact sort ear and keeps group-by-realm in navigation', () => {
    ...
    expect(
      screen.getByRole('button', {name: /toggle grouping awakeners by realm/i}),
    ).toBeInTheDocument()
  })
  ...
  it('hides awakener level editor for unowned awakeners', () => {
    ...
    expect(
      screen.queryByRole('button', {name: /edit awakener level for ramona/i}),
  ...
  it('opens wheel and posse details from public entity refs', async () => {
    render(<CollectionPage />)
  ...
  it('freezes wheel card order after enlighten edits until apply changes is clicked', () => {
    ...
    fireEvent.click(
      within(callOfTheDeepCard as HTMLElement).getByRole('button', {
        name: /increase enlighten level/i,
      }),
    )
  ...
  it('uses click-to-edit awakener level with Lv. prefix inside editor', () => {
    ...
    expect(
      screen.queryByRole('textbox', {name: /awakener level for ramona/i}),
    ).not.toBeInTheDocument()
  ...
  it('does not keep swallow state after non-card outside click commit', () => {
    ...
    expect(
      screen.queryByRole('button', {name: /edit awakener level for ogier/i}),
    ).not.toBeInTheDocument()
    vi.useRealTimers()
  })
  ...
  it('keeps edit mode open when pressing a disabled level chevron', () => {
    ...
    fireEvent.click(screen.getByRole('button', {name: /edit awakener level for ramona/i}))
    const disabledIncrease = screen.getByRole('button', {
      name: /increase awakener level for ramona/i,
    })
  ...
  it('applies wheel enlighten +12 batch action to filtered wheels', () => {
    ...
    const firstWheelCard = screen
      .getByRole('button', {name: /toggle ownership for birth of a soul/i})
      .closest('.collection-card-frame')
    const secondWheelCard = screen
      .getByRole('button', {name: /toggle ownership for call of the deep/i})
      .closest('.collection-card-frame')
    ...
    expect(
      firstWheelScope.getByRole('button', {name: /decrease enlighten level/i}),
    ).not.toBeDisabled()
    expect(secondWheelScope.getByRole('button', {name: /increase enlighten level/i})).toBeDisabled()
    expect(
      secondWheelScope.getByRole('button', {name: /decrease enlighten level/i}),
    ).not.toBeDisabled()
  })
