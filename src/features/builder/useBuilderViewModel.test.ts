...
describe('useBuilderViewModel', () => {
  ...
  it('removes an active awakener selection from a slot', () => {
    ...
    expect(result.current.teamSlots[0]?.awakenerId).toBeUndefined()
    expect(result.current.activeSelection).toBeNull()
  })
  ...
  it('tracks used awakeners by identity key across teams', () => {
      ...
      result.current.setTeams([
        {
          ...
          slots: firstTeam.slots.map((slot, index) =>
            index === 0 ? {...slot, awakenerId: 'awakener-0042', realm: 'CHAOS'} : slot,
          ),
        },
        ...rest,
      ])
    ...
    expect(
      result.current.usedAwakenerIdentityKeys.has(getAwakenerIdentityKeyById('awakener-0042')),
    ).toBe(true)
    expect(
      result.current.usedAwakenerIdentityKeys.has(getAwakenerIdentityKeyById('awakener-0020')),
    ).toBe(true)
  })
  ...
  it('keeps the current quick lineup wheel step active when removing that wheel', () => {
    ...
    expect(result.current.teamSlots[0]?.awakenerId).toBe('awakener-0021')
  ...
  it('jumps quick lineup to the drop target when swapping active team slots', () => {
    ...
    expect(result.current.teamSlots[1]?.awakenerId).toBe('awakener-0021')
    expect(result.current.quickLineupSession?.currentStep).toEqual({
      kind: 'awakener',
      slotId: 'slot-2',
    })
