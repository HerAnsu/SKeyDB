...
describe('BuilderPage covenants', () => {
  ...
  it('does not render database detail shortcuts on active card covenant tiles', () => {
    ...
    fireEvent.click(screen.getByRole('button', {name: /set covenant/i}))
    fireEvent.click(screen.getByRole('button', {name: /deus ex machina covenant/i}))
    const activeCardCovenantTile = screen
      .getByRole('button', {name: /edit covenant/i})
      .closest('.covenant-tile')
    expect(activeCardCovenantTile).toBeInstanceOf(HTMLElement)
    if (!(activeCardCovenantTile instanceof HTMLElement)) {
  ...
  it('renders the unset covenant slot with the svg placeholder frame', () => {
    ...
    expect(screen.queryByTitle(/open not set details overlay/i)).not.toBeInTheDocument()
  })
