...
function makeWheelFullRecord(overrides: Partial<WheelFullRecord> = {}): WheelFullRecord {
  return {
    id: 'B01',
    assetId: 'Weapon_Full_B01',
    name: 'Merciful Nurturing',
...
describe('WheelDetailModal', () => {
  ...
  it('clamps the enhance slider within supported wheel bounds', () => {
    ...
    expect(
      screen.getAllByText(resolveWheelMainstatValue(fullData.mainstatSeriesKey, 15)).length,
    ).toBeGreaterThan(0)
  ...
  it('shows a stable no-image fallback when wheel art is unavailable', () => {
    mockGetWheelAssetById.mockReturnValue(undefined)
