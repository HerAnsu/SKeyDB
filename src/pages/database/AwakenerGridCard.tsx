...
export function AwakenerGridCard({ awakener, onSelect }: AwakenerGridCardProps) {
  ...
  return (
    <article className="collection-item-card group/collection p-1">
      <div
        className="collection-card-frame relative aspect-[5/9] overflow-hidden border border-slate-400/35 bg-slate-900/75 transition-[border-color,box-shadow] duration-150 group-hover/collection:border-amber-200/45 group-hover/collection:shadow-[0_0_0_1px_rgba(251,191,36,0.15)]"
      >
        <button
          aria-label={`View details for ${displayName}`}
          className="absolute inset-0 z-[13]"
          onClick={() => onSelect(awakener.id)}
          type="button"
        />
        {cardAsset ? (
          <img
        ...
        />
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[11] bg-gradient-to-t from-slate-950/92 via-slate-950/70 to-transparent px-1.5 pb-1.5 pt-5">
          ...
          {awakener.stats ? (
            <div className="mt-0.5 flex items-center gap-2">
              {STAT_DISPLAY.map(({ key, color }) => {
                ...
                return (
                  <span className="inline-flex items-center gap-0.5 text-[10px]" key={key} style={{ color }}>
                    {icon ? <img alt={key} className="h-3 w-3 opacity-85" draggable={false} src={icon} /> : null}
                    {awakener.stats![key]}
                  </span>
                )
              })}
