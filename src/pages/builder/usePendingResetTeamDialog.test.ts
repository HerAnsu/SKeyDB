...
describe('usePendingResetTeamDialog', () => {
  it('resets empty teams immediately without pending dialog', () => {
    ...
    const {result} = renderHook(() => {
      const [teams, setTeams] = useState<readonly Team[]>(initialTeams);
    ...
    expect(
      result.current.teams[1]?.slots.every((slot) => !slot.awakenerName),
    ).toBe(true);
  ...
  it('requires confirmation for non-empty active team and clears selection on confirm', () => {
    const initialTeams = [
      buildTeam('team-1', 'Team 1', 'goliath', 'taverns-opening'),
    ];
    const {result} = renderHook(() => {
      const [teams, setTeams] = useState<readonly Team[]>(initialTeams);
      const [selectionCleared, setSelectionCleared] = useState(false);
    ...
    expect(
      result.current.teams[0]?.slots.every((slot) => !slot.awakenerName),
    ).toBe(true);
