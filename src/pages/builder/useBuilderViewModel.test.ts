...
describe('useBuilderViewModel', () => {
  ...
  it('initializes with a valid active team and slots', () => {
    ...
    expect(result.current.effectiveActiveTeamId).toBe(
  ...
  it('tracks used awakeners by identity key across teams', () => {
      ...
      result.current.setTeams([
        {
          ...
          slots: firstTeam.slots.map((slot, index) =>
            index === 0
              ? {...slot, awakenerName: 'Ramona', realm: 'CHAOS'}
              : slot,
          ),
        },
        ...rest,
    ...
    expect(
      result.current.usedAwakenerIdentityKeys.has(
        getAwakenerIdentityKey('Ramona'),
      ),
    ).toBe(true);
    expect(
      result.current.usedAwakenerIdentityKeys.has(
        getAwakenerIdentityKey('Ramona: Timeworn'),
      ),
    ).toBe(true);
  });
  ...
  it('filters wheels by rarity and searches wheel metadata fields', () => {
    ...
    expect(
      result.current.filteredWheels.every((wheel) => wheel.rarity === 'R'),
    ).toBe(true);

    act(() => {
      result.current.setWheelRarityFilter('ALL');
      result.current.setPickerSearchByTab((prev) => ({
        ...prev,
        wheels: 'ultra',
      }));
    });
    expect(result.current.filteredWheels.length).toBeGreaterThan(0);
    expect(
      result.current.filteredWheels.every((wheel) => wheel.realm === 'ULTRA'),
    ).toBe(true);
  ...
  it('sorts wheels by rarity, then realm (Chaos first), then id', () => {
    ...
    for (let index = 1; index < wheels.length; index += 1) {
      ...
      expect(
        prev.id.localeCompare(next.id, undefined, {
          numeric: true,
          sensitivity: 'base',
        }),
  ...
  it('autosaves team draft changes to localStorage', () => {
    ...
    const storedRaw = requireDefined(
      window.localStorage.getItem(BUILDER_PERSISTENCE_KEY),
    );
  ...
  it('hydrates ownership and keeps linked awakeners synced', () => {
    ...
    expect(
      result.current.ownedAwakenerLevelByName.get('ramona: timeworn'),
    ).toBe(5);
  });
  ...
  it('defaults team preview mode to compact and persists expanded mode', () => {
    ...
    expect(window.localStorage.getItem(BUILDER_TEAM_PREVIEW_MODE_KEY)).toBe(
      'expanded',
    );
  });
  ...
  it('starts quick lineup by snapshotting and clearing the active team, then activating the first awakener step', () => {
    ...
    act(() => {
      ...
      result.current.updateActiveTeam((team) => ({
        ...team,
  ...
  it('hydrates and persists builder awakener sort config', () => {
    ...
    expect(window.localStorage.getItem(BUILDER_AWAKENER_SORT_KEY_KEY)).toBe(
      'LEVEL',
    );
    expect(
      window.localStorage.getItem(BUILDER_AWAKENER_SORT_DIRECTION_KEY),
    ).toBe('DESC');
    expect(
      window.localStorage.getItem(BUILDER_AWAKENER_SORT_GROUP_BY_REALM_KEY),
    ).toBe('0');
  });
});
