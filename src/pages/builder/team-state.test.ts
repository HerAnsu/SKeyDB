...
describe('builder team state', () => {
  ...
  it('moves assignment when target slot is valid', () => {
    ...
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-1')?.awakenerName,
    ).toBeUndefined();
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-2')?.awakenerName,
    ).toBe('Goliath');
  });
  ...
  it('swaps source and target slot payloads', () => {
    ...
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-1')?.wheels,
    ).not.toBe(slots[1].wheels);
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-2')?.wheels,
    ).not.toBe(slots[0].wheels);
  });
  ...
  it('treats alternate forms as globally unique within the team', () => {
    const slots = teamSlotsForTests();
    const withRamona = assignAwakenerToSlot(
      slots,
      'Ramona',
      'slot-2',
      awakenersByNameForTests,
    ...
    expect(
      withTimeworn.nextSlots.find((slot) => slot.slotId === 'slot-2')
        ?.awakenerName,
    ).toBeUndefined();
    expect(
      withTimeworn.nextSlots.find((slot) => slot.slotId === 'slot-3')
        ?.awakenerName,
    ).toBe('Ramona: Timeworn');
  });
  ...
  it('allows replacing an alternate form in the same slot', () => {
    ...
    expect(
      replacedWithBase.nextSlots.find((slot) => slot.slotId === 'slot-2')
        ?.awakenerName,
    ).toBe('Ramona');
  });
  ...
  it('does not move an already slotted awakener when adding to first empty slot', () => {
    ...
    const result = assignAwakenerToFirstEmptySlot(
      slots,
      'Goliath',
      awakenersByNameForTests,
    );
    expect(result.nextSlots).toBe(slots);
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-3')?.awakenerName,
    ).toBe('Goliath');
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-4')?.awakenerName,
    ).toBeUndefined();
  });
  ...
  it('clears a filled slot when removing via picker dropzone', () => {
    ...
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-1')?.realm,
    ).toBeUndefined();
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-1')?.level,
    ).toBeUndefined();
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-1')?.wheels,
    ).toEqual([null, null]);
  ...
  it('assigns and clears wheel values on a specific wheel slot', () => {
    ...
    expect(
      withWheel.nextSlots.find((slot) => slot.slotId === 'slot-2')?.wheels,
    ).toEqual([null, 'demo-wheel']);
    expect(
      clearedWheel.nextSlots.find((slot) => slot.slotId === 'slot-2')?.wheels,
    ).toEqual([null, null]);
  });
  ...
  it('blocks assigning wheels to slots without an awakener', () => {
    ...
    expect(result.nextSlots).toBe(slots);
    expect(
      result.nextSlots.find((slot) => slot.slotId === 'slot-3')?.wheels,
    ).toEqual([null, null]);
  });
  ...
  it('returns same slots when assigning an unchanged wheel value', () => {
    ...
    const unchanged = assignWheelToSlot(
      withWheel.nextSlots,
      'slot-1',
      0,
      'wheel-a',
  ...
  it('swaps wheels correctly when source and target are on the same slot', () => {
    ...
    const withTwoWheels = assignWheelToSlot(
      withFirstWheel.nextSlots,
      'slot-1',
      1,
      'wheel-b',
    );
  ...
  it('assigns and clears covenant values on an awakener slot', () => {
    ...
    expect(
      withCovenant.nextSlots.find((slot) => slot.slotId === 'slot-1')
        ?.covenantId,
    ).toBe('001');
    expect(
      cleared.nextSlots.find((slot) => slot.slotId === 'slot-1')?.covenantId,
    ).toBeUndefined();
  });
  ...
  it('returns same slots when assigning an unchanged covenant value', () => {
    const slots = teamSlotsForTests();
    const withCovenant = assignCovenantToSlot(slots, 'slot-1', '001');
    const unchanged = assignCovenantToSlot(
      withCovenant.nextSlots,
      'slot-1',
      '001',
    );
