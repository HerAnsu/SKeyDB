...
describe('useBuilderDndCoordinator', () => {
  ...
  it('routes preview-slot drag start to preview-slot handler only', () => {
    const {coordinator, onDragStart, onTeamPreviewSlotDragStart} =
      createCoordinator();
    const event = {
      active: {
        data: {
    ...
    expect(onTeamPreviewSlotDragStart).toHaveBeenCalledWith(
      'team-1',
      'team-1-slot-1',
    );
    expect(onDragStart).not.toHaveBeenCalled();
  ...
  it('routes preview-slot drag over to preview-slot handler only', () => {
    const {coordinator, onDragOver, onTeamPreviewSlotDragOver} =
      createCoordinator();
    const event = {
      active: {
        data: {
    ...
    expect(onTeamPreviewSlotDragOver).toHaveBeenCalledWith(
      'dropzone:team-preview-slot:team-2:team-2-slot-1',
    );
    expect(onDragOver).not.toHaveBeenCalled();
  });

  it('reorders teams on team-row drag end with valid ids', () => {
    const {coordinator, onTeamRowDragEnd, onTeamRowReorder, onDragEnd} =
      createCoordinator();
    const event = {
      active: {data: {current: {kind: 'team-row', teamId: 'team-1'}}},
      over: {id: 'team-2'},
  ...
  it('routes preview-slot drag end to preview-slot handler only', () => {
    const {coordinator, onDragEnd, onTeamPreviewSlotDragEnd} =
      createCoordinator();
    const event = {
      active: {
        data: {
    ...
    expect(onTeamPreviewSlotDragEnd).toHaveBeenCalledWith(
      'team-1',
      'team-1-slot-1',
      'dropzone:team-preview-slot:team-2:team-2-slot-1',
    );
