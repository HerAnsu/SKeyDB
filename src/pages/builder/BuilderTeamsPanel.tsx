...
import {BuilderTeamRow} from '@/pages/builder/BuilderTeamRow';
import {MAX_TEAMS, type TeamTemplateId} from '@/pages/builder/team-collection';
import type {Team, TeamPreviewMode} from '@/pages/builder/types';
import {SortableContext, verticalListSortingStrategy} from '@dnd-kit/sortable';

export interface BuilderTeamsPanelProps {
  readonly teams: readonly Team[];
  readonly activeTeamId: string;
  readonly editingTeamId: string | null;
  readonly editingTeamName: string;
  readonly editingTeamSurface: 'header' | 'list' | null;
  readonly posses: readonly Posse[];
  ...
  readonly teamPreviewMode: TeamPreviewMode;
  readonly onAddTeam: () => void;
  readonly onApplyTeamTemplate: (templateId: TeamTemplateId) => void;
  readonly onExportTeam: (teamId: string) => void;
  readonly onResetTeam: (teamId: string, teamName: string) => void;
  readonly onTeamPreviewModeChange: (nextMode: TeamPreviewMode) => void;
  ...
  readonly onEditTeam: (teamId: string) => void;
  readonly onDeleteTeam: (teamId: string, teamName: string) => void;
}
        ...
        <div className='mt-2 space-y-2'>
          {teams.map((team) => {
            const isEditingTeamName =
              editingTeamId === team.id && editingTeamSurface === 'list';
