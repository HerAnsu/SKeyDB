...
export type TeamStateViolationCode =
  | 'TOO_MANY_REALMS_IN_TEAM'
  | 'INVALID_BUILD_RULES';

export interface TeamStateUpdateResult {
  readonly nextSlots: readonly TeamSlot[];
  readonly violation?: TeamStateViolationCode;
}

function asRealmMembers(slots: readonly TeamSlot[]) {
  return slots
    .filter((slot): slot is TeamSlot & {realm: string} =>
      Boolean(slot.awakenerName && slot.realm),
    )
    .map((slot) => ({realm: slot.realm}));
}

export function getTeamRealmSet(slots: readonly TeamSlot[]): Set<string> {
  return new Set(
    asRealmMembers(slots).map((member) => member.realm.trim().toUpperCase()),
  );
}
...
export function assignAwakenerToSlot(
  ...
  const sourceSlotId = options?.allowDuplicateIdentity
    ...
    : currentSlots.find(
        (slot) =>
          slot.awakenerName &&
          getAwakenerIdentityKey(slot.awakenerName) === sourceIdentityKey,
      )?.slotId;
...
export function assignAwakenerToFirstEmptySlot(
  ...
  const firstEmptySlotId = currentSlots.find(
    (slot) => !slot.awakenerName,
  )?.slotId;
  if (!firstEmptySlotId) {
    return {nextSlots: currentSlots};
  }

  return assignAwakenerToSlot(
    currentSlots,
    awakenerName,
    firstEmptySlotId,
    awakenerByName,
