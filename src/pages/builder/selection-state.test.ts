...
describe('selection-state', () => {
  it('toggles awakener selection for the same slot', () => {
    expect(
      toggleAwakenerSelection({kind: 'awakener', slotId: 'slot-1'}, 'slot-1'),
    ).toBeNull();
    expect(toggleAwakenerSelection(null, 'slot-1')).toEqual({
      kind: 'awakener',
      slotId: 'slot-1',
    });
  });
  ...
  it('toggles wheel selection for the same slot/index', () => {
    ...
    expect(toggleWheelSelection(null, 'slot-1', 1)).toEqual({
      kind: 'wheel',
      slotId: 'slot-1',
      wheelIndex: 1,
    });
  ...
  it('enables wheel-active landing only when wheel selection is active', () => {
    ...
    expect(
      shouldSetActiveWheelOnPickerAssign({kind: 'awakener', slotId: 'slot-1'}),
    ).toBe(false);
