...
interface InternalQuickLineupSession {
  ...
  readonly originalTeam: Team;
  readonly currentStepIndex: number;
  readonly steps: readonly QuickLineupStep[];
...
export function buildQuickLineupSteps(
  slots: readonly TeamSlot[],
): readonly QuickLineupStep[] {
  return [
...
export function createQuickLineupSession(
  team: Team,
...
export function getQuickLineupStepSelection(
...
export function findNextQuickLineupStepIndex(
  session: InternalQuickLineupSession,
  currentSlots: readonly TeamSlot[],
): number | null {
  const currentStep = getQuickLineupStepAtIndex(
    session,
    session.currentStepIndex,
  ...
  if (currentStep.kind !== 'awakener') {
    return session.currentStepIndex + 1 < session.steps.length
      ? session.currentStepIndex + 1
      : null;
  }

  const currentSlot = currentSlots.find(
    (slot) => slot.slotId === currentStep.slotId,
  );
  ...
  for (
    ...
    if (
      session.steps[index]?.kind === 'awakener' ||
      session.steps[index]?.kind === 'posse'
    ) {
      return index;
    }
  }
...
export function reconcileQuickLineupSessionAfterSlotsChange(
  ...
  const targetStep =
    preferredStep ??
    getQuickLineupStepAtIndex(session, session.currentStepIndex);
  if (!targetStep || targetStep.kind === 'posse') {
    return session;
  }

  const targetSlot = nextSlots.find(
    (slot) => slot.slotId === targetStep.slotId,
  );
  const reconciledStep =
    targetStep.kind === 'awakener' || targetSlot?.awakenerName
      ...
      : ({
          kind: 'awakener',
          slotId: targetStep.slotId,
        } satisfies QuickLineupStep);
