...
describe('OwnedWheelBoxExport', () => {
  it('defaults rarity filters to SSR only', () => {
    ...
    fireEvent.click(
      screen.getByRole('button', {name: /export wheels as png/i}),
    );

    expect(screen.getByLabelText('Include SSR wheels')).toHaveAttribute(
      'data-checked',
      'true',
    );
    expect(screen.getByLabelText('Include SR wheels')).toHaveAttribute(
      'data-checked',
      'false',
    );
    expect(screen.getByLabelText('Include R wheels')).toHaveAttribute(
      'data-checked',
      'false',
    );
