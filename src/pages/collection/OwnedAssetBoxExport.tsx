...
export interface ExportBoxConfig {
  ...
  readonly levelTextScalePct: number;
  readonly outerPaddingXPx: number;
  readonly outerPaddingYPx: number;
  readonly pixelRatio: number;
}

export interface ExportVisualConfig {
  readonly disableNames: boolean;
  readonly nameOnTop: boolean;
  readonly enlightensOnCard: boolean;
  readonly showLevels: boolean;
  readonly disableEmoji: boolean;
}

export interface OwnedAssetBoxEntry<TRarity extends string = string> {
  readonly id: string;
...
export type ExportSortBehavior = 'CONFIGURABLE' | 'WHEEL_DEFAULT';
...
export interface OwnedAssetBoxExportProps<R extends string = string> {
  readonly entries: readonly OwnedAssetBoxEntry<R>[];
  readonly onStatusMessage: (message: string) => void;
  readonly storageKeyPrefix: string;
  readonly buttonLabel: string;
  readonly modalTitle: string;
  readonly filenamePrefix: string;
  readonly nameToggleLabel?: string;
  readonly assetAltNoun: string;
  readonly cardAspectClassName: string;
  readonly imageClassName: string;
  readonly placeholderClassName: string;
  readonly rarityOptions?: readonly RarityOption<R>[];
  readonly defaultIncludedRarities?: Record<R, boolean>;
  readonly sortBehavior?: ExportSortBehavior;
...
const VALID_SORT_KEYS: readonly AwakenerSortKey[] = [
  'ALPHABETICAL',
  'LEVEL',
  'RARITY',
  'ENLIGHTEN',
];
const VALID_SORT_DIRECTIONS: readonly CollectionSortDirection[] = [
  'ASC',
  'DESC',
];
...
function loadStoredSortConfig(storageKeyPrefix: string): ExportSortConfig {
  ...
  try {
    ...
    const rawKey = typeof parsed.key === 'string' ? parsed.key : undefined;
    const rawDirection =
      typeof parsed.direction === 'string' ? parsed.direction : undefined;
    const legacyGroupByRealm =
      typeof parsed.groupByFaction === 'boolean'
        ? parsed.groupByFaction
        : undefined;

    let groupByRealm = DEFAULT_EXPORT_SORT_CONFIG.groupByRealm;
    ...
    const isValidKey =
      rawKey && (VALID_SORT_KEYS as readonly string[]).includes(rawKey);
    const isValidDirection =
      rawDirection &&
      (VALID_SORT_DIRECTIONS as readonly string[]).includes(rawDirection);

    return {
      key: isValidKey
        ? (rawKey as AwakenerSortKey)
        : DEFAULT_EXPORT_SORT_CONFIG.key,
      direction: isValidDirection
        ? (rawDirection as CollectionSortDirection)
...
interface ExportPreviewProps<R extends string> {
  readonly config: ExportBoxConfig;
  readonly visuals: ExportVisualConfig;
  readonly emojiAsset: string | null;
  readonly entries: readonly OwnedAssetBoxEntry<R>[];
  readonly assetAltNoun: string;
  readonly cardAspectClassName: string;
  readonly imageClassName: string;
...
function ExportPreview<R extends string>({
  ...
  const levelSizePx = Math.round(
    clamp(config.cardWidthPx * 0.155, 10, 24) * levelScaleMultiplier,
  );
  ...
  return (
    ...
    >
      ...
      >
        {entries.map((entry) => (
          <article
            className='border border-slate-500/45 bg-slate-900/65 p-1'
            key={entry.id}
          >
            <div
            ...
            >
              ...
              {(() => {
                const hasBottomLevel =
                  visuals.showLevels && typeof entry.cardLevel === 'number';
                    ...
                    {hasBottomLevel ? (
                      <p
                        ...
                        style={{
                          ...
                          textShadow:
                            '0 1px 2px rgba(2, 6, 12, 0.9), 0 0 6px rgba(2, 6, 12, 0.65)',
                        }}
                      >
                        <span
                          style={{fontSize: `${String(levelPrefixSizePx)}px`}}
                        >
                          Lv.
                        </span>
...
function useExportEntries<R extends string>(
  entries: readonly OwnedAssetBoxEntry<R>[],
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter((entry) =>
      entry.rarity ? includedRarities[entry.rarity] : true,
    );
  ...
  const sortedEntries = useMemo(() => {
    ...
    return [...filteredEntries].sort((left, right) => {
      ...
      if (sortBehavior === 'WHEEL_DEFAULT') {
        return compareWheelsForCollectionDefaultSort(
          leftSortable,
          rightSortable,
        );
      }
  ...
  return {filteredEntries, sortedEntries};
}
...
async function performPngExport(
  ...
  try {
    dataUrl = await withTimeout(
      toPng(element, baseRenderOptions),
      20000,
      'PNG render',
    );
  } catch (error) {
  ...
  if (!dataUrl) {
    throw new Error('renderer returned empty image data.');
  }
...
interface ExportSettingsSidebarProps<R extends string> {
  readonly rarityOptions?: readonly RarityOption<R>[];
  readonly includedRarities: Record<R, boolean> | null;
  readonly sortedEntriesLength: number;
  readonly hasSortControls: boolean;
  readonly sortConfig: ExportSortConfig;
  readonly setSortConfig: React.Dispatch<
    React.SetStateAction<ExportSortConfig>
  >;
  readonly supportsRealmGrouping: boolean;
  readonly selectedSortKey: AwakenerSortKey;
  readonly sortOptions: readonly AwakenerSortKey[];
  readonly nameToggleLabel?: string;
  readonly assetAltNoun: string;
  readonly visuals: ExportVisualConfig;
  readonly setVisuals: React.Dispatch<React.SetStateAction<ExportVisualConfig>>;
  readonly supportsLevels: boolean;
  readonly isEmojiInTitleEnabled: boolean;
  readonly rerollEmoji: () => void;
  readonly draftConfig: ExportBoxConfig;
  readonly setDraftConfig: React.Dispatch<
    React.SetStateAction<ExportBoxConfig>
  >;
  readonly sanitizedDraftConfig: ExportBoxConfig;
  readonly handleRarityToggle: (rarity: R, checked: boolean) => void;
}

function ExportSettingsSidebar<R extends string>({
  rarityOptions,
  includedRarities,
  sortedEntriesLength,
  hasSortControls,
  ...
  setVisuals,
  supportsLevels,
  isEmojiInTitleEnabled,
  rerollEmoji,
  draftConfig,
  setDraftConfig,
  sanitizedDraftConfig,
  handleRarityToggle,
}: ExportSettingsSidebarProps<R>) {
    ...
    <div className='collection-scrollbar space-y-2 overflow-auto border border-slate-500/45 bg-slate-900/50 p-3 text-xs text-slate-300'>
      {rarityOptions && includedRarities ? (
        <div className='space-y-2 border border-slate-700/60 bg-slate-950/50 p-2'>
          <p className='text-[11px] tracking-wide text-slate-300 uppercase'>
            Rarities
          </p>
        ...
        <div className='space-y-2 border border-slate-700/60 bg-slate-950/50 p-2'>
          <CollectionSortControls
            ...
            onGroupByRealmChange={(checked) => {
              setSortConfig((c) => ({...c, groupByRealm: checked}));
            }}
            onSortDirectionToggle={() => {
              setSortConfig((c) => ({
                ...c,
                direction: c.direction === 'DESC' ? 'ASC' : 'DESC',
              }));
            }}
            onSortKeyChange={(nextKey) => {
              setSortConfig((c) => ({...c, key: nextKey}));
            }}
      ...
      <div className='space-y-2 border border-slate-700/60 bg-slate-950/50 p-2'>
          ...
          <TogglePill
            ariaLabel={
              visuals.disableNames
                ? `Show ${nameToggleLabel ?? assetAltNoun} names on card`
                : `Hide ${nameToggleLabel ?? assetAltNoun} names on card`
            }
            checked={!visuals.disableNames}
            className='ownership-pill-builder'
            offLabel='Off'
            onChange={(checked) => {
              setVisuals((c) => ({...c, disableNames: !checked}));
            }}
            onLabel='On'
            variant='flat'
          />
        ...
        {areNamesEnabled ? (
          <div className='flex items-center justify-between gap-2'>
            ...
            <TogglePill
              ariaLabel={
                visuals.nameOnTop ? 'Set names to bottom' : 'Set names to top'
              }
              checked={visuals.nameOnTop}
              className='ownership-pill-builder'
              offLabel='Off'
              onChange={(nameOnTop) => {
                setVisuals((c) => ({...c, nameOnTop}));
              }}
              onLabel='On'
              variant='flat'
            />
        ...
        <div className='flex items-center justify-between gap-2'>
          ...
          <TogglePill
            ...
            onChange={(enlightensOnCard) => {
              setVisuals((c) => ({...c, enlightensOnCard}));
            }}
            onLabel='On'
            variant='flat'
          />
        ...
        <div className='flex items-center justify-between gap-2'>
          ...
          <TogglePill
            ariaLabel={
              isEmojiInTitleEnabled
                ? 'Disable emoji in title'
                : 'Enable emoji in title'
            }
            checked={isEmojiInTitleEnabled}
            className='ownership-pill-builder'
            offLabel='Off'
            onChange={(emojiInTitleEnabled) => {
              setVisuals((c) => ({...c, disableEmoji: !emojiInTitleEnabled}));
              rerollEmoji();
            }}
            onLabel='On'
            variant='flat'
      ...
      <ExportSliderField
        ...
        onChange={(cardWidthPx) => {
          setDraftConfig((c) => ({...c, cardWidthPx}));
        }}
      ...
      {supportsLevels ? (
        <ExportSliderField
          ...
          onChange={(levelTextScalePct) => {
            setDraftConfig((c) => ({...c, levelTextScalePct}));
          }}
          step={5}
      ...
      <ExportSliderField
        ...
        onChange={(outerPaddingXPx) => {
          setDraftConfig((c) => ({...c, outerPaddingXPx}));
        }}
      ...
      <ExportSliderField
        ...
        onChange={(outerPaddingYPx) => {
          setDraftConfig((c) => ({...c, outerPaddingYPx}));
        }}
      ...
      <ExportSliderField
        ...
        min={0.5}
        onChange={(pixelRatio) => {
          setDraftConfig((c) => ({...c, pixelRatio}));
        }}
        step={0.1}
        value={draftConfig.pixelRatio}
      />
      <p className='text-[11px] text-slate-400'>
        Pixel ratio controls render density. Higher values look sharper, but
        increase file size quite a bit.
      </p>
...
export function OwnedAssetBoxExport<R extends string>({
  ...
  const [includedRarities, setIncludedRarities] = useState<Record<
    R,
    boolean
  > | null>(() => {
    ...
    return loadStoredIncludedRarities(
      storageKeyPrefix,
      defaultIncludedRarities,
    );
  });
  const [emojiAsset, setEmojiAsset] = useState<string | null>(() =>
    emojiAssets.length > 0
      ? emojiAssets[getSecureRandomIndex(emojiAssets.length)]
      : null,
  );
  const previewRef = useRef<HTMLDivElement | null>(null);

  const sanitizedDraftConfig = useMemo(
    () => sanitizeConfig(draftConfig),
    [draftConfig],
  );
  ...
  const {sortedEntries} = useExportEntries(
    entries,
    includedRarities,
    sortBehavior,
    sortConfig,
  ...
  const hasSortControls =
    sortBehavior === 'CONFIGURABLE' && sortOptions.length > 0;
  ...
  const hasAtLeastOneRarity = includedRarities
    ...
    : true;
  let exportUnavailableReason: string | null = null;
  if (!hasAtLeastOneRarity) {
    exportUnavailableReason = 'Enable at least one rarity to export.';
  } else if (sortedEntries.length === 0) {
    exportUnavailableReason = 'Nothing matches the current filters.';
  }
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      `${storageKeyPrefix}.rarities.v1`,
      JSON.stringify(includedRarities),
    );
  ...
  function rerollEmoji() {
    ...
    setEmojiAsset(emojiAssets[getSecureRandomIndex(emojiAssets.length)]);
  }
  ...
  async function handleExport() {
    ...
    if (sortedEntries.length === 0) {
      onStatusMessage(
        'PNG export skipped: nothing to export with current filters.',
      );
      return;
    }
    ...
    try {
      ...
      const detail = error instanceof Error ? error.message : String(error);
      onStatusMessage(
        `PNG export failed: ${detail || 'could not render image.'}`,
      );
    } finally {
        ...
        >
          <div className='mt-3 grid min-h-0 flex-1 gap-3 lg:grid-cols-[340px_1fr]'>
            <ExportSettingsSidebar
              ...
              isEmojiInTitleEnabled={isEmojiInTitleEnabled}
              ...
              sortOptions={sortOptions}
              sortedEntriesLength={sortedEntries.length}
