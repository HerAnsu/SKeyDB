...
import { parseRichDescription, getCardNamesFromFull } from './rich-text'
...
describe('parseRichDescription', () => {
  ...
  it('parses scaling with decimal values', () => {
    const result = parseRichDescription('Deal (12.5/15/17.5/20/22.5/25% {DEF}) Shield.', EMPTY_CARDS)
    expect(result).toEqual([
      { type: 'text', value: 'Deal ' },
  ...
  it('handles scaling before bracket tokens in the same string', () => {
    const result = parseRichDescription('Deal (10/12/14/16/18/20% {ATK}) DMG. Inflict {Poison}.', EMPTY_CARDS)
    expect(result).toEqual([
      { type: 'text', value: 'Deal ' },
      { type: 'scaling', values: [10, 12, 14, 16, 18, 20], suffix: '%', stat: 'ATK' },
  ...
  it('parses a complex real-world description', () => {
    ...
    expect(result[1]).toEqual({ type: 'scaling', values: [25, 30, 35, 40, 45, 50], suffix: '', stat: null })
    ...
    expect(result[5]).toEqual({ type: 'scaling', values: [8, 9.6, 11.2, 12.8, 14.4, 16], suffix: '%', stat: 'ATK' })
