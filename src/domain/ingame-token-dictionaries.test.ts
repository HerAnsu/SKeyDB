...
} from '@/domain/ingame-token-dictionaries';
...
describe('buildTokenDictionaryFromEntries', () => {
  ...
  it('reports duplicate tokens and excludes ambiguous mappings', () => {
    ...
    expect(
      result.issues.some((issue) => issue.kind === 'duplicate_token'),
    ).toBe(true);
  });
  ...
  it('reports missing ids from the canonical catalog', () => {
    ...
    expect(
      result.issues.some(
        (issue) => issue.kind === 'missing_token_for_id' && issue.id === 'b',
      ),
    ).toBe(true);
  });
  ...
  it('reports unknown source ids', () => {
    ...
    expect(
      result.issues.some(
        (issue) => issue.kind === 'unknown_source_id' && issue.id === 'z',
      ),
    ).toBe(true);
  });
});
...
describe('buildIngameTokenDictionaries', () => {
  ...
  it('uses canonical posse ids rather than display names in the tracked token file', () => {
    ...
    expect(
      result.issues.some(
        (issue) =>
          issue.category === 'posses' && issue.kind === 'unknown_source_id',
      ),
    ).toBe(false);
  });
