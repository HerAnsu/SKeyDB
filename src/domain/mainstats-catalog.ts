...
const mainstatLabels = new Set<string>()
for (const mainstat of parsedMainstats) {
  mainstatLabels.add(mainstat.label)
  for (const alias of mainstat.aliases) {
    mainstatLabels.add(alias)
  }
}
...
export function getMainstatLabels(): Set<string> {
  return new Set(mainstatLabels)
}
