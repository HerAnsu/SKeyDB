...
export function getRealmLabel(realmId: string): string {
  const normalizedId = normalizeRealmId(realmId);

  if (normalizedId in REALM_LABEL_BY_ID) {
    return REALM_LABEL_BY_ID[normalizedId as keyof typeof REALM_LABEL_BY_ID];
  }
