...
describe('decodeIngameTeamCode', () => {
  ...
  it('keeps import recoverable and reports warnings for unsupported covenant/posse blocks', () => {
    ...
    expect(decoded.team.slots.every((slot) => Boolean(slot.awakenerName))).toBe(
      true,
    );
    expect(
      decoded.warnings.some((warning) => warning.section === 'covenant'),
    ).toBe(true);
    expect(
      decoded.warnings.some((warning) => warning.section === 'covenant'),
    ).toBe(true);
  ...
  it('matches observed in-game export layout for no-covenant team with posse token', () => {
    ...
    expect(encodeIngameTeamCode(team)).toBe(
      '@@UliXxW5aaxY1xVxDaaaaaaaaaaaaaaaaaaaaaaaa3@@',
    );
  });
