...
| C4: Shared `WheelSlotIndex` type | TypeScript scouts | queued | `src/features/builder/types.ts`, V2 model/types, quick-lineup/DnD data types, focused V1/V2 tests | Shared `QuickLineupStep`, `ActiveSelection`, and `WheelUsageLocation` use `number`; V2 clamps malformed values to `1`. | Root-fix candidate before DnD; touches shared builder types, so Judge must protect V1 behavior. |
...
| C6: Pure picker option query modules | React/complexity scouts | queued | `useBuilderV2Model.ts`, new picker query module(s), `BuilderV2ModelTypes.ts`, focused model tests | Four picker option pipelines are large `useMemo` blocks coupled to hook state and hard to test directly. | J4 active; C6 scout recommends pure `builder-v2-picker-options.ts` plus direct helper tests. |
| C7: Stable action groups / adaptive callback churn | React scouts | queued | `useBuilderV2Model.ts`, `BuilderV2Page.tsx`, `BuilderV2AdaptiveLayout.tsx`, focused page tests | Desktop wraps assignment callbacks with `useStableEvent`; adaptive callbacks close over the entire `model`. | Candidate may be removed by C2 if command groups become stable. |
...
| C9: Shared picker a11y hardening | UI/a11y scouts | queued | `BuilderV2AwakenerPicker.tsx`, `BuilderV2Page.test.tsx`, maybe CSS | Shared picker content reuses fixed DOM IDs across desktop and adaptive instances; tile `aria-label`s omit blocked/unowned/recommended/active statuses. | Small high-leverage UI/a11y Worker candidate. |
| C10: Adaptive drawer focus containment/restore | UI/a11y scouts | queued | `BuilderV2AdaptiveLayout.tsx`, focused page tests | Assignment close path suppresses focus restore; focus trap lacks outside-activeElement fallback. | Queue as UI/a11y hardening; do not change drawer shape. |
| C11: Builder V2 motion/focus CSS contract | CSS/UI scouts | queued | `builder-v2.css`, Browser visual checks, focused a11y/manual checks | Reduced motion only covers drawer animations; focus styles are partly global and partly bespoke. | Queue as CSS/a11y slice. |
| C12: Rail/button CSS dedupe | CSS scouts | queued | `builder-v2.css`, `BuilderV2Page.tsx`, `BuilderV2AdaptiveLayout.tsx` only if class names change | Desktop/adaptive rail buttons and button recipes duplicate nearly identical styles. | Queue after command boundary or as CSS-only cleanup. |
...
| C14: Picker grid sizing contract | CSS/UI scouts | queued | `builder-v2.css`, Browser checks, page tests if needed | Desktop picker uses fixed 4-column tracks while adaptive/mobile override via ancestor selectors. | Queue as CSS slice if overflow recurs or before drawer polish. |
| C15: Persistence envelope parser | TypeScript scout | queued | `src/features/builder/builder-persistence.ts`, migrations tests | `JSON.parse(raw) as PersistedBuilderEnvelope` casts before envelope validation. | In scope only if Judge selects shared persistence hardening; otherwise lower priority because current goal is Builder V2 internals. |
...
Current tranche decision: W1 implemented C1 Assignment behavior characterization, W2 implemented C2 Builder V2 loadout command resolver extraction, and W3 implemented C3 editing-target boundary. J4 is active for C6 picker query extraction.
