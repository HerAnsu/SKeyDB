...
| Zustand/Immer state strategy | reviewed | J12/R11 reviewed `builderDraftStore`, `useBuilderV2Model`, command modules, and dependency policy. Zustand already owns durable draft state; no evidence showed Immer or a store rewrite is the current root fix. | No state-library shift in this goal. Revisit with `$refactor-dependencies` only if future DnD/store evidence proves command boundaries are insufficient. |
...
| C5: Explicit team-state operation result | Complexity scouts | implemented | `src/features/builder/team-state.ts`, V1/V2 callers, team-state tests, V2 model tests | Callers use `result.nextSlots === activeTeamSlots` as noop control flow. | Implemented by W12 with additive `changed` metadata, shared helper tests, and V2 no-op handling routed through `result.changed`. |
...
| C13: Clipping/drag overlay CSS boundary | CSS scouts | queued | `builder-v2.css`, maybe future drag overlay root | `.builder-v2-panel` and cards use clipping for both panel containment and art masks, which can clip future drag ghosts/focus halos. | Keep queued as first-DnD-tranche prep once a concrete drag overlay root or ghost surface exists to validate against; do not add DnD here. |
...
| C16: Zustand/Immer state strategy review | User prompt | implemented | Read-only review of `builderDraftStore`, `useBuilderV2Model`, command candidates, dependency policy | Review found Zustand is already used for the shared draft store and Immer is installed, but current Builder V2 pain is better addressed by command/projection/input-boundary slices than by state-library redesign. | Implemented by J12/R11 as a review-only decision: no Zustand/Immer strategy shift for this goal; revisit only with DnD evidence and explicit dependency/state-architecture approval. |
...
Current tranche decision: W1 implemented C1 Assignment behavior characterization, W2 implemented C2 Builder V2 loadout command resolver extraction, W3 implemented C3 editing-target boundary, W4 implemented C6 picker query extraction, W8 implemented C4, W9 implemented C11/C14, W10 implemented C15, W11 implemented C12, and W12 implemented C5 while J12/R11 closed C16 as a review-only state-strategy decision. J13 is active only if the next tranche starts concrete DnD-prep around C8/C13.
