...
| Current builder contract reuse | implemented | W1 added `useBuilderV2Model` as a thin facade over `builderDraftStore`, current persistence, ownership hydration, search helpers, and `team-state` assignment/removal helpers. | Extend the facade for gear, posse, import/export, and transfer parity in queued tranches. |
| Old UX branch flow salvage | mapped | Scout identified mobile overview/focused/picker/quick-lineup and tablet/desktop flow as reference only. | Use in responsive layout tranches, not first draft loop slice. |
| Test/characterization harness | implemented | W1 added V2 model/page outcome tests and kept current builder helper/integration tests green. | Add focused characterization with each later workflow slice. |
| Concept image alignment | implemented | W1 follows `Desktop.png` anatomy with left team rail, center active builder, lower team overview, and right picker/armory; mobile smoke checked the single-column collapse. | Mobile drawer and quick-lineup concepts remain queued as C5/C6. |
...
| C4: Gear and posse assignment | S1 | queued | V2 slot loadout UI, wheel/covenant/posse action helpers, transfer dialogs | Core must support wheels, covenants, posse, ownership, recommendations. | Follow base awakeners slice. |
| C5: Mobile overview/focused/picker drawer flow | UX branch scout | queued | V2 mobile layout files, drawer/focus management, focused slot UI tests | `builder/mobile-ux` mobile flow is the strongest salvageable part. | Defer until desktop/tablet shell has working core actions. |
| C6: Quick Lineup V2 mode | UX/tests scouts | queued | V2 quick-lineup UI, current `quick-lineup.ts`, store quick-lineup state, tests | Current helpers and old branch flow both support guided stepper. | Defer until picker/gear assignment works. |
| C7: Teams overview redesign | User prompt | queued | V2 teams surface, team-management helpers, screenshot/share needs | User is undecided between list/cards; core builder first. | Keep minimal in early tranches; require product decision later. |
| C8: Import/export and transfer parity | Contract/test scouts | queued | V2 dialogs, import/export hooks, transfer confirm hooks, tests | Current page tests pin these workflows; helpers exist but UI blast radius is larger. | Defer after core draft loop and gear/posse interactions. |
