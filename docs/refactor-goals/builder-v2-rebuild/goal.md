...
| Current builder contract reuse | implemented | W1 added `useBuilderV2Model` as a thin facade over `builderDraftStore`, current persistence, ownership hydration, search helpers, and `team-state` assignment/removal helpers. W2 extended the facade to wheels, covenants, and team-level posse assignment. | Extend the facade for import/export, transfer parity, DnD, quick-lineup, and recommendations in queued tranches. |
